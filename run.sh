...
if [ $DISTIMAGE -eq 1 ]; then
    ...
    docker run --rm -p 3000:3000 --env-file app/.env dist
    exit 0
fi
...
if [ $TESTS -eq 1 ]; then
    ...
    # docker compose exec app /bin/bash -c "npm run test"
    exit 0
fi

if [ $DEBUG -eq 1 ]; then
    echo "Debug mode not supported for the Next.js container yet."
    exit 1
fi

if [ $EXEC -eq 1 ]; then
    # Run application
    docker compose exec app /bin/bash -c "npm run dev"
else
    # Log into container
