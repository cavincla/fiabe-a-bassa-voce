...
if [[ ! -z "$FIX_UID" ]] && [[ "$FIX_UID" != "$DOCKER_USER_UID" ]]; then
    ...
    if [ ! -z "$PREV_USER" ]; then
        ...
        find / -user ${FIX_UID} -exec chown -h 888 {} \; 2>/dev/null || true
        usermod -u 888 $PREV_USER
    fi

    find / -user ${DOCKER_USER_UID} -exec chown -h ${FIX_UID} {} \; 2>/dev/null || true
...
if [[ ! -z "$FIX_GID" ]] && [[ "$FIX_GID" != "$DOCKER_GROUP_GID" ]]; then
    ...
    if [ ! -z "$PREV_GROUP" ]; then
        ...
        find / -group ${FIX_GID} -exec chgrp -h 888 {} \; 2>/dev/null || true
        groupmod -g 888 $PREV_GROUP
    fi

    find / -group ${DOCKER_GROUP_GID} -exec chgrp -h ${FIX_GID} {} \; 2>/dev/null || true
